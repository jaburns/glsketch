...
void teapot_renderer_use(TeapotRenderer *renderer, const vec3 *camera_pos, const mat4x4 *view, const mat4x4 *projection)
{
    ...
    glUniform3fv(glGetUniformLocation(renderer->program, "camera_pos"), 1, (const float*)camera_pos);
    glUniformMatrix4fv(glGetUniformLocation(renderer->program, "view"), 1, GL_FALSE, (const float *)view);
...
void teapot_renderer_draw(TeapotRenderer *renderer, const Transform *transform)
{
    ...
    mat4x4_from_quat(r, (float *)transform->rotation); // TODO linmath functions should accept const stuff correctly
    mat4x4_mul(m, m, r);

    glUniformMatrix4fv(glGetUniformLocation(renderer->program, "model"), 1, GL_FALSE, (const float *)m);
